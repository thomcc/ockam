defmodule Ockam.Protocol.Tests do
  ...
  describe "Protocol message" do
    test "can be encoded and decoded" do
      ...
      struct_request_encoded = Protocol.encode(ExampleProtocol, :request, {:structure, struct_request})
      assert is_binary(struct_request_encoded)
      assert {:ok, {:structure, ^struct_request}} = Protocol.decode(ExampleProtocol, :request, struct_request_encoded)

      data_request_encoded = Protocol.encode(ExampleProtocol, :request, {:data, data_request})
      assert {:ok, {:data, ^data_request}} = Protocol.decode(ExampleProtocol, :request, data_request_encoded)

      data_response_encoded = Protocol.encode(ExampleProtocol, :response, data_response)
      assert {:ok, ^data_response} = Protocol.decode(ExampleProtocol, :response, data_response_encoded)

      assert catch_error(Protocol.encode(ExampleProtocol, :response, struct_request)) == :cannot_encode
    end
  end
  ...
  describe "Protocol message wrapped in the base message" do
    test "can be encoded and decoded" do
      ...
      struct_request_encoded = Protocol.encode_payload(ExampleProtocol, :request, {:structure, struct_request})
      assert is_binary(struct_request_encoded)

      assert {:ok, %{protocol: "example_protocol"}} = Protocol.base_decode(struct_request_encoded)

      assert {:ok, {:structure, ^struct_request}} = Protocol.decode_payload(ExampleProtocol, :request, struct_request_encoded)
    end
  end
end