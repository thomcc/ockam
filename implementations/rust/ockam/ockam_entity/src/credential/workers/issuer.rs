use crate::{
    check_message_origin, CredentialAttribute, CredentialProtocolMessage, CredentialSchema, Issuer,
...
impl IssuerWorker {
    pub fn new(
        ...
        let s = Self {
            profile,
            state: State::Offer(return_route),
            holder_id,
            schema,
        };
...
impl Worker for IssuerWorker {
    ...
    async fn initialize(&mut self, ctx: &mut Self::Context) -> Result<()> {
        if let State::Offer(return_route) = &self.state {
